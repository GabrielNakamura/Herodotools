...
```{r echo=TRUE}
library(BioGeoBEARS)
load(file = here::here("inst", "extdata", "resDEC_akodon.RData")) # ancestral reconstruction
...
map_dispersal_E <- 
   sites %>% 
   ggplot() + 
   geom_raster(aes(x = LONG, y = LAT, fill = dispersal.E)) + 
   scale_fill_carto_c(type = "quantitative", palette = "SunsetDark") +
   geom_sf(data = coastline, size = 0.4) +
   geom_sf(
     data = sf.evoregion, 
...
map_dispersion_from <- map_evoregion / (map_dispersal_A | map_dispersal_B | map_dispersal_E) +
...
require(here)
rodent.phylo <- ape::read.nexus(here("inst", "extdata","Sigmodontinae_285speciesTree.tre"))
  
# traits
traits <- read.table(here ("inst","extdata" , "MamFuncDat.txt"),
                     header = T, sep = "\t")
...
# load function
source (here ("R", "transition_rates_function.R"))
...
load(here::here("inst", "extdata", "transition_rate_res.RData"))
```
...
# load community data
comm.data <- read.table(here ("inst","extdata" , "PresAbs_228sp_Neotropical_MainDataBase_Ordenado.txt"),
                     header = T, sep = "\t")

# load latlong of these communities

geo.data <- read.table(here ("inst","extdata" , "Lon-lat-Disparity.txt"),
                     header = T, sep = "\t")
...
mTR_community <- lapply (seq (1,nrow(comm.data)), function (i) # across assemblages 
  
            lapply (trans_rates, function (sims) { # across simulations
...
# melt the list to have a dataframe 
# each element in the list is one assemblage, and the rows within each element represent the number of simulations
mTR_community_DF <- lapply (mTR_community, function (i)
  
  do.call (rbind, i) 
  
  )

# average rates (across the simulations) per assemblage
averaged_rates <- lapply (mTR_community_DF, function (comm) 
  
  apply (comm, 2,mean)
  
)
averaged_rates <- do.call(rbind, averaged_rates) # melt to have ncells x tip-based metrics


# prepare data to map
# bind tip-based metrics to geographic data
data_to_map <- data.frame(geo.data[,c("LONG", "LAT")],
                          averaged_rates)


# reshape to long format (suitable for ggplot)
require(reshape)
melt_data_to_map <- melt(data_to_map,id=c("LONG","LAT"))
colnames(melt_data_to_map)[which(colnames(melt_data_to_map) == "value")] <- 'mTR'

```

We also added an elevation raster to plot in the same figure the elevation and the tip-based metrics across rodent assemblages. The spatial distribution of tip-based metrics showed that assemblages in the southern, western and northeastern Neotropics have species with higher Transition Rates in foraging strata than communities from elsewhere (i.e., they have many species that often changed their foraging strata over time). Assemblage-level Stasis Time was high for two groups of assemblages: one in northeastern Neotropics, and another in central Amazonia. The former also showed high Transition Rates. Together, these results indicate that, despite the frequent changes in foraging strata, the species of northeastern assemblages retained during more time, in average, a given state of foraging strata than the species found elsewhere. Finally, the Last Transition Time metric shows that the transitions leading to the present species foraging strata were older in the i) north of the Amazonia and ii) central Mexico than elsewhere. These results are potentially reflecting i) the place of arrival of ancestral sigmodontine rodents in south America, and ii) the occurrence of several species closely related to basal lineages of sigmodontine rodents.

```{r , echo=T}

require(raster)

rel <- raster(here("inst", "extdata", "DEM_Elev.tif"))
...
maps <- lapply (unique(melt_data_to_map$variable), function (i) { # one map per tip-based metric
    
  sel_data = melt_data_to_map[which(melt_data_to_map$variable == i),]
  
  map1 <- ggplot(sel_data, 
                 aes(x = LONG, y = LAT)) +
    geom_raster(data = rel, aes_string(x = "x", y = "y", 
                                       alpha = "DEM_Elev"),
                show.legend=F) +
    geom_tile(aes(fill = mTR),alpha=0.65)+
    #facet_wrap(~variable,scales = "fixed",ncol=3)+
    scale_fill_gradient2(midpoint = mean(sel_data$mTR),
                         limits=c(range(sel_data$mTR)[1],
                                  range(sel_data$mTR)[2]),
                         breaks=seq(range(sel_data$mTR)[1],
                                    range(sel_data$mTR)[2],
                                    diff(range(sel_data$mTR))/4),
                          low="#F2EBE9",mid="#FAC213", high="#D61C4E") + 
    theme_classic() + 
    theme_map() +
...
require(gridExtra)
mapTransition_rate <- 
  grid.arrange(maps[[1]]+ggtitle ("A) Transition Rates")+theme(plot.title = element_text(size=17)),
             maps[[2]]+ggtitle ("B) Stasis Time")+theme(plot.title = element_text(size=17)),
             maps[[3]] +ggtitle ("C) Last Transition Time")+theme(plot.title = element_text(size=17)),
                                 ncol=3)
