...
Here we will show how to implement the three tip-based metrics idealized by [Luza et al. (2021)] (https://onlinelibrary.wiley.com/doi/full/10.1002/ece3.8476). Note that these tip-based metrics were estimated using the stochastic mappingof discrete/categorical traits (diet in the original publication, but can be extended to other traits as we will show here). The three tip-based metrics are *Transition rates*, *Stasis time*, and *Last transition time*, and are implemented in the function *tip.based.trait.evo*. 
Transition rates indicate how many times the ancestral character has changed over time. Stasis time indicates the maximum branch length (time interval) over which the current tip-character was maintained across the whole phylogeny. Finally, last transition time is the sum of branch lengths from the tip to the prior/previous node with a reconstructed character equal to current tip-character.
As in the original publication, we will use the stochastic character mapping on discrete trait data. This time, however, we will reconstruct the species foraging strata (Elton Traits' database, Wilman et al. 2014) rather than diet. We will reconstruct the foraging strata for 214 sigmodontine rodent species with trait and phylogenetic data (consensus phylogeny of Upham et al. 2019).  


First we will load trait and phylogenetic data we need to run the function *tip.based.trait.evo*.
```{r , echo=T, eval=TRUE}
...
# run function
trans_rates <- tip.based.trait.evo (tree=match_data$phy,
...
Now that we have in hand the estimates of the three tip-based metrics across the 214 rodent species, we can summarize the tip-based metrics at the ecological assemblage scale. First we will load assemblage & geographic data comprising 1770 grid cells spread into the Neotropics.
...
Now we then gather the values of tip-based metrics for all species present in a given ecological assemblage *i*.
...
We will do an additional step of loading a elevation raster to set in the background of the map, and then represent in space the variation in the tip-based metrics across rodent assemblages. 
...
```{r , echo=T, eval=TRUE, fig.height=7,fig.width=18,fig.align= "center",fig.margin=T}
