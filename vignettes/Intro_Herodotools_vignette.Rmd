...
Historical events affects our daily lives in many ways. Who has never asked himself what would happened if we have done something different in the past? Like in the classic movie by Frank Capra [It's a Wonderful Life](https://en.wikipedia.org/wiki/It%27s_a_Wonderful_Life), that latter was the inspiration for the book name by the great [SJ Gould](https://en.wikipedia.org/wiki/Wonderful_Life_(book)), changes in past events could change dramatically our present life. The same is true in the natural world, history matters a lot, and the distribution of species in ecological communities is a result of present and past events. 

Like the ancient Greek historian and geographer [Herodotus](https://en.wikipedia.org/wiki/Herodotus), our package aims to put together tools that allows to investigate in a systematic way the role of history, but instead of narrating kings and famous battles, we hope to help ecologist in explaining the natural world.
...
5. [Calculate](#traitdynamics) ti/model-based metrics of trait macroevolutionary dynamics
...
ggplot2::ggsave(plot = map_richness,  filename = "Fig1_richmap_akodon.png", path = here::here("vignettes"), dpi = "retina")
...
sf.evoregion <- terra::as.polygons(r_evoregion) %>% 
  sf::st_as_sf()
...
sites <- bind_cols(site_xy, site.region =  site.region, afi)
...
a_region <- Herodotools::region_occ(comm = akodon.pa.tree, site.region = site.region)
...
```
It is worth to note that the procedures decribed from here will work with any model of ancestral area reconstruction, but for sake of simplicity we decided to use only the [DEC model](https://revbayes.github.io/tutorials/biogeo/biogeo_intro.html). 
...
biogeo_area <- data.frame(biogeo = chartr("12345", "ABCDE", evoregion_df$site.region)) 
...
age_comm <- Herodotools::calc_age_arrival(W = akodon.pa.tree, 
                        tree = akodon.newick, 
...
sites <- dplyr::bind_cols(site_xy, site.region =  site.region, age = age_comm$mean_age_per_assemblage)
...
Herodotools::calc_model_based_diversification(W = akodon.pa.tree,
                   tree = akodon.newick, 
                   ancestral.area = node_area, 
                   biogeo = biogeo_area, 
...
akodon_dispersal <- 
Herodotools::calc_dispersal_from(W = akodon.pa.tree,
               tree = akodon.newick, 
               ancestral.area = node_area, 
               biogeo = biogeo_area)
...
akodon_db_metrics <- 
Herodotools::calc_div_based_metrics(W = akodon.pa.tree,
                  tree = akodon.newick, 
                  ancestral.area = node_area, 
                  biogeo = biogeo_area, 
                  PD = TRUE, 
...
# run function
trans_rates <- calc_tip_based_trait_evo(tree = match_data$phy,
