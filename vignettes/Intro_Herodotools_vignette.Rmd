...
```{r outFig4, echo=FALSE, eval=FALSE,out.width="70%",fig.cap="Age of assemblages"}
map_joint_evoregion_afilliation
```
...
```
```{r outFig5, echo=FALSE, eval=FALSE,out.width="70%",fig.cap="Diversification rates (DR - A) and in situ diversification rate (DRinsitu - B) for Akodon assemblages"}
...
```{r outFig6, echo=FALSE,eval=FALSE,fig.cap="Maps showing regionalization based on phylogenetic turnover (evoregion - top figure), and the contribution of regions A, B and D to other regions regarding historical dispersal of lineages",out.width="70"}
...
```{r fig6_PEdb, echo=FALSE, eval=FALSE,out.width="70%"}
map_PE_all
```
...
rodent_phylo <- data("rodent_phylo")
traits <- data("trait")
```
