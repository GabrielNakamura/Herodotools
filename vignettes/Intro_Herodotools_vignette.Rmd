...
The object created in the last step can be used in an auxiliary function present in Herodotools to easily produce the [Phyllip](http://scikit-bio.org/docs/0.2.3/generated/skbio.io.phylip.html) file required to run the analysis of ancestral area reconstruction using BioGeoBears.
...
TheSince we have an occurrence data distribution, a biogeographical regionalization and the ancestral area reconstruction we can integrate those three information to calculate metrics that represents historical variables at any spatial scale.
...
map_diversification_model_based <- 
  ...
  ggtitle("C") + 
  theme_bw() +
  labs(fill = "In-situ diversification") +
  guides(fill = guide_colorbar(barheight = unit(2.3, units = "mm"),  
                               direction = "horizontal",
                               ticks.colour = "grey20",
                               title.position = "top",
...
maps_complete <- map_evoregion + map_age + map_diversification_model_based + map_dispersal_D
