...
#'
#' @param x A matrix containing the continuous probabilities of occurrences of 
...
find_threshold <- function(x, 
  ...
  if (length(steps_off) > 0) {
    ...
    threshold.steps <- threshold.steps
  }
  ...
  return(list(Maximum_site_probability = max,
              Removed_sites = sites_off,
              Correlation_between_steps = cor_steps,
              Correlation_between_steps = eigen,
              Select_threshold = sel))
}